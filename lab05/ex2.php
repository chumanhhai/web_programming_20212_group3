  ...
  <body>
    ...
    <form action="ex2.php" method="POST">
      ...
      if (!$connect) {
        ...
        $business_query = "CREATE TABLE if not exists businesses (
        ...
        $biz_category_query = "CREATE TABLE if not exists biz_categories (
        ...
        if ($connect->query($business_query)) {
          if (!$connect->query($biz_category_query)) {
      ...
      if (!empty($name) && !empty($address) && !empty($city) && !empty($phone) && !empty($URL) && !empty($categories) ) {
        ...
        if (!mysqli_query($connect, $insertBusinessQuery) ) {
          ...
          foreach ($categories as $category) {
            $query = "INSERT INTO biz_categories VALUES ('$businessId', '$category')";
